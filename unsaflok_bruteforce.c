...
typedef struct {
    ...
    char uid_input[32];
    char dict_input[128];
    // For variable list enter callback
...
// -------- Attack thread (now matches FuriThreadCallback) --------
static int32_t attack_loop(void* context) {
    ...
    if(index == VarListKnownUID) {
        ...
        text_input_set_result_callback(app->text_input,
            (TextInputCallback)[](void* ctx) {
                ...
                variable_item_set_current_value_text(a->known_uid_item, display);
        ...
        text_input_set_result_callback(app->text_input,
            (TextInputCallback)[](void* ctx) {
                ...
                variable_item_set_current_value_text(a->dict_path_item, a->temp_config.dict_path);
                view_dispatcher_switch_to_view(a->view_dispatcher, 2);
            }, app);
...
static void build_settings_view(UnsaflokApp* app) {
    ...
    // Known UID (clickable via list enter callback)
    ...
    // Dict path (clickable via list enter callback)
